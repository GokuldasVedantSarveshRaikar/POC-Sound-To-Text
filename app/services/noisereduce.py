...
def estnoisem(pSpectrum,hop_length):
    ...
    aminh=np.exp(-hop_time/taminh) # min smoothing constant (upper limit) in (3) = 0.3
    ...
    nsms=np.array([10.0])**np.clip(nsmdb*nv*hop_time/10, -50.0, 50.0)  # [8 4 2 1.2] in paper
    ...
    for t in range(0,nFrames):                       # we use t instead of lambda in the paper
        ...
        acb=(1+(sum(p) / (sum(pSpectrum_t) + eps)-1)**2)**(-1)    # alpha_c-bar(t)  (9)
        ...
        ah=amax*ac*(1+(p/sn2_safe-1)**2)**(-1)       # alpha_hat: smoothing factor per frequency (11)
        ...
        qeqi=max_complex(min_complex(qeqi_raw,np.array([qeqimax] )),np.array([qeqimin/(t+1)] ))   # Qeq inverse (23)
        ...
        bmind=1+2*(nd-1)*(1-md)/bmind_denom      # we use the signalmplified form (17) instead of (15)
        bminv_denom = qeqi_inv - 2*mv
        bminv_denom = np.where(np.abs(bminv_denom) < eps, eps, bminv_denom)
        bminv=1+2*(nv-1)*(1-mv)/bminv_denom    # same expressignalon but for sub windows
        ...
        xs[t,:]=sn2*np.sqrt(0.266*(nd+100*qisq)*qisq/(1+0.005*nd+6/nd)/xs_denom)
